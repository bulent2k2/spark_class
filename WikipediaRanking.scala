...
object WikipediaRanking extends WikipediaRankingInterface {
  ...
  val conf: SparkConf = new SparkConf().setMaster("local").setAppName("Programming Language Analysis Based on Wikipedia Articles")
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = rdd.
    aggregate(0)((count: Int,  wa: WikipediaArticle) => if (wa.mentionsLanguage(lang)) 1 + count else count, _ + _ )
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] = rdd
    .flatMap( wa => for ( l <- langs; if (wa.mentionsLanguage(l))) yield (l, wa) )
    .groupByKey()
  ...
  def rankLangsUsingIndex(index: RDD[(String, Iterable[WikipediaArticle])]): List[(String, Int)] = index
    .mapValues(_.size)
    .sortBy({ case (lang: String, count: Int) => count }, ascending = false)
    .collect().toList
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = rdd
    .flatMap( wa => for ( l <- langs; if (wa.mentionsLanguage(l))) yield (l, 1) )
    .reduceByKey( _ + _ )
    .sortBy( { case (lang:  String, count: Int) => count }, ascending = false)
    .collect().toList
